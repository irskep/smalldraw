...
describe('KonvaReconciler', () => {
  ...
  test('reconciler updates only dirty shapes', () => {
    ...
    reconciler.reconcile(layer, shapes, new Set(['shape-1', 'shape-2']), new Set());

    const node1Before = reconciler.getNode('shape-1');
    const node2Before = reconciler.getNode('shape-2');

    // Update only shape-1
    shapes[0] = { ...shapes[0], transform: { ...shapes[0].transform!, translation: { x: 50, y: 50 } } };
    reconciler.reconcile(layer, shapes, new Set(['shape-1']), new Set());
  ...
  test('reconciled render matches full render', () => {
    ...
    reconcileDocument(stageB, reconciler, shapes, allDirty, { viewport: baseViewport });
  ...
  test('reconciled render after move updates node position', () => {
    ...
    reconcileDocument(stage, reconciler, shapes, allDirty, { viewport: baseViewport });
    ...
    reconcileDocument(stage, reconciler, shapes, moveDirty, { viewport: baseViewport });
  ...
  test('reconciled render after delete removes node', () => {
    ...
    reconcileDocument(stage, reconciler, initialShapes, allDirty, { viewport: baseViewport });
    ...
    reconcileDocument(stage, reconciler, remainingShapes, deleteDirty, { viewport: baseViewport });
