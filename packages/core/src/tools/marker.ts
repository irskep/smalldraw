...
const runtimeState = new WeakMap<ToolRuntime, ActiveMarkerState>();
...
export function createMarkerTool(options?: MarkerToolOptions): ToolDefinition {
  ...
  const ensureState = (runtime: ToolRuntime): ActiveMarkerState => {
  ...
  const resolveStroke = (runtime: ToolRuntime): StrokeStyle => {
    const runtimeOptions = runtime.getOptions<MarkerToolOptions>();
    ...
    return {
      ...
      brushId: override?.brushId ?? fallbackStroke.brushId ?? "marker",
      compositeOp:
  ...
  const beginDrawing = (
    ...
    const draftId = runtime.generateShapeId("marker-draft");
  ...
  const finishStroke = (runtime: ToolRuntime) => {
    ...
    if (shape) {
      shape.id = runtime.generateShapeId("marker");
