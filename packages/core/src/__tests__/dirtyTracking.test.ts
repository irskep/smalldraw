...
import { AddShape, CompositeAction, DeleteShape, UpdateShapeTransform, UpdateShapeZIndex } from '../actions';
...
describe('Dirty tracking', () => {
  ...
  test('affectedShapeIds returns union for CompositeAction', () => {
    const action = new CompositeAction([
      new UpdateShapeTransform('shape-1', { translation: { x: 0, y: 0 } }),
      new UpdateShapeTransform('shape-2', { translation: { x: 0, y: 0 } }),
      new UpdateShapeTransform('shape-1', { translation: { x: 10, y: 10 } }),
    ]);
  ...
  test('multiple mutations accumulate in dirty state', () => {
    ...
    store.mutateDocument(new UpdateShapeTransform('shape-1', {
...
describe('Ordered shapes caching', () => {
  test('getOrderedShapes returns shapes in zIndex order', () => {
    const store = new DrawingStore({ tools: [] });
    store.mutateDocument(new AddShape({ ...createTestShape('shape-1'), zIndex: 'b0' }));
    store.mutateDocument(new AddShape({ ...createTestShape('shape-2'), zIndex: 'a0' }));
  ...
  test('cache is invalidated after UpdateShapeZIndex', () => {
    const store = new DrawingStore({ tools: [] });
    store.mutateDocument(new AddShape({ ...createTestShape('shape-1'), zIndex: 'a0' }));
    store.mutateDocument(new AddShape({ ...createTestShape('shape-2'), zIndex: 'b0' }));
